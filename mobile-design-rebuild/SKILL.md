...
这是用户确认用的 HTML 审阅文档，必须包含：

- 规格版本、状态、日期、源设计图、目标页面、目标端、当前审批状态。
- 原始设计图预览；参考裁剪图必须标注 `仅参考，不是最终资产`。
- 输入契约、平台倾向判断、页面定位证据、新增/重构判断。
- 页面风格、背景、颜色、字体、间距、圆角、阴影、图标和图片规则。
- 多端换算表：iOS pt、Android dp/sp、H5 CSS px/rem。
- 组件注册表和每页结构树。
- 素材清单：完全复用、相似但拒绝、缺失需生图、参考图、已生成候选。
- 生图候选审阅区：候选图、处理后图片、contact sheet、主线程选择结果。
- 开放问题、不确定项、验收标准和用户确认区。
